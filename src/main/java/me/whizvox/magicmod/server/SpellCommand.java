...
public class SpellCommand {
  ...
  private static int list(CommandSourceStack src) throws CommandSyntaxException {
    ...
    var knownSpells = magicUser.knownSpells().toList();
  ...
  private static int unlearn(CommandSourceStack src, Spell spell) throws CommandSyntaxException {
    ServerPlayer player = src.getPlayerOrException();
    MagicUser magicUser = MagicUserManager.getUser(player);
    magicUser.unlearnSpell(spell);
    if (magicUser.hasBeenModified()) {
  ...
  public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
    var builder = Commands.literal("spell")
        ...
        .then(Commands.literal("unlearn")
            .then(Commands.argument("spell", SpellArgumentType.spell())
                .executes(ctx -> unlearn(ctx.getSource(), SpellArgumentType.getSpell(ctx, "spell")))
            )
        )
